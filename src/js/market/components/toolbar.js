...
function currentTitle() {
  ...
  return (
    state.categories.find((c) => c.id === state.filters.category)?.name ??
    "Tous les produits"
  );
}
...
function renderChips() {
  ...
  state.filters.producers.forEach((id) => {
    chips.push(
      chip(
        "producer",
        id,
        findProducer(id)?.name ?? findProducer(id)?.contact ?? "",
      ),
    );
  });
  ...
  if (state.filters.maxPrice < PRICE_CEILING) {
    chips.push(
      chip("price", "", `Jusqu'à ${formatPrice(state.filters.maxPrice)}`),
    );
  }
...
function removeFilter(type, value) {
  ...
  if (type === "zone")
    state.filters.zones = state.filters.zones.filter((z) => z !== value);
  if (type === "producer")
