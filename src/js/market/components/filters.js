...
const countBy = (except, predicate) =>
  filterProducts(except).filter(predicate).length;
...
export function renderFilters(container) {
  ...
  const producerList = state.producers.filter((producer) =>
    (producer.name ?? producer.contact ?? "")
      .toLowerCase()
      .includes(state.producerQuery.toLowerCase()),
  );
  const visibleProducers = state.producersExpanded
    ? producerList
    : producerList.slice(0, 4);
...
function bind() {
  ...
  root.querySelectorAll('input[name="zone"]').forEach((input) => {
    input.addEventListener("change", () => {
      state.filters.zones = toggleValue(
        state.filters.zones,
        input.value,
        input.checked,
      );
  ...
  root.querySelectorAll('input[name="producteur"]').forEach((input) => {
    input.addEventListener("change", () => {
      state.filters.producers = toggleValue(
  ...
  maxInput.addEventListener("change", () => {
    const value = Math.min(
      Math.max(Number(maxInput.value) || PRICE_CEILING, 500),
      PRICE_CEILING,
    );
